...
export type MezoGaugeLockTxState = {
  tokenId: bigint
...
export default function useMezoGaugeVoting() {
  const { chainId } = useNetwork()
  const { address: account } = useAccount()
  const { data: walletClient } = useWalletClient({ chainId })
  const { sendTransactionAsync } = useSendTransaction()
  const publicClient = usePublicClient({ chainId })
  const voter = getContractConfig(chainId).thirdPartyVoter
  const safeBatch = useSafeBatchExport("mezo-gauge-vote")
  const [lockStates, setLockStates] = useState<MezoGaugeLockTxState[]>([])
  ...
  const exportVoteBatch = useCallback(
    async (tokenIds: bigint[], gauges: Address[], weights: bigint[]) => {
      ...
      const pending = await safeBatch.exportBatch({
        name: `Matchbox MEZO gauge vote ${tokenIds.length} locks`,
        description: "veMEZO MEZO gauge vote generated by Matchbox.",
        calls,
  ...
  const exportResetBatch = useCallback(
    async (tokenIds: bigint[]) => {
      const pending = await safeBatch.exportBatch({
        name: `Matchbox MEZO gauge unpair ${tokenIds.length} locks`,
        description: "veMEZO MEZO gauge reset generated by Matchbox.",
        calls: tokenIds.map((tokenId) =>
          encodeSafeBatchCall({
            to: voter.address,
            abi: voter.abi,
        ...
        items: tokenIds.map((tokenId) => ({
          id: tokenId.toString(),
          label: `Unpair veMEZO #${tokenId.toString()}`,
        })),
      })
      if (pending) setExecutionMode("safe-export")
    },
  ...
  const copyResetBatchJson = useCallback(
    (tokenIds: bigint[]) =>
      safeBatch.copyBatchJson({
        name: `Matchbox MEZO gauge unpair ${tokenIds.length} locks`,
        description: "veMEZO MEZO gauge reset generated by Matchbox.",
